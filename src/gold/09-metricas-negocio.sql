...
-- Genie acertar de primeira em vez de tentar adivinhar qual JOIN fazer.
--
-- O COMMENT de cada view diz QUAL PERGUNTA ela responde, não o que ela é.
-- É assim que o Genie escolhe onde procurar.
...
CREATE OR REPLACE VIEW lakehouse_rotaperfume.gold.margem_por_categoria (
  ...
  margem_tabela_pct COMMENT 'Margem teórica do catálogo (preço de tabela menos custo, sobre preço de tabela). A diferença para a praticada é o efeito do desconto comercial.',
...
CREATE OR REPLACE VIEW lakehouse_rotaperfume.gold.clientes_em_risco (
  ...
  dias_sem_comprar      COMMENT 'Dias entre o último pedido do cliente e 31/08/2026, a data de referência do dataset.',
...
SELECT c.cliente_id, c.razao_social, c.segmento, c.cidade,
       c.data_ultimo_pedido AS ultimo_pedido,
       c.dias_sem_comprar,
       c.total_pedidos,
